...
(function() {
	...
	require.register("abpetkov-transitionize/transitionize.js", function(
		...
		/**
		 * Transitionize 0.0.3 https://github.com/abpetkov/transitionize
		 * 
	...
	require
			.register(
					...
					function(exports, require, module) {
						/**
						 * @preserve FastClick: polyfill to remove click delays
						 *           on browsers with touch UIs.
						 * 
						 * @version 0.6.11
						 * @codingstandard ftlabs-jsv2
						 * @copyright The Financial Times Limited [All Rights
						 *            Reserved]
						...
						/**
						 * Instantiate fast-clicking listeners on the specificed
						 * layer.
						 * 
						 * @constructor
						 * @param {Element}
						...
						function FastClick(layer) {
							...
							/**
							 * ID of the last touch, retrieved from
							 * Touch.identifier.
							 * 
							 * @type number
							 */
							...
							/**
							 * Touchmove boundary, beyond which a click will be
							 * cancelled.
							 * 
							 * @type number
							 */
							...
							// which is how FastClick normally stops click
							// events bubbling to callbacks registered on the
							// FastClick
							...
							// FastClick's onClick handler. Fix this by pulling
							// out the user-defined handler function and
							// adding it as listener.
							if (typeof layer.onclick === 'function') {

								// Android browser on at least 3.2 requires a
								// new reference to the function in
								// layer.onclick
						...
						/**
						 * iOS 6.0(+?) requires the target element to be
						 * manually derived
						 * 
						 * @type boolean
						 */
						...
						/**
						 * Determine whether a given element requires a native
						 * click.
						 * 
						 * @param {EventTarget|Element}
						 *            target Target DOM element
						 * @returns {boolean} Returns true if the element needs
						 *          a native click
						 */
						FastClick.prototype.needsClick = function(target) {
							...
							case 'input':
								...
								// File inputs need real clicks on iOS 6 due to
								// a browser bug (issue #68)
						...
						/**
						 * Determine whether a given element requires a call to
						 * focus to simulate click into element.
						 * 
						 * @param {EventTarget|Element}
						 *            target Target DOM element
						 * @returns {boolean} Returns true if the element
						 *          requires a call to focus to simulate native
						 *          click.
						 */
						FastClick.prototype.needsFocus = function(target) {
							...
							case 'input':
								...
								// No point in attempting to focus disabled
								// inputs
						...
						FastClick.prototype.sendClick = function(targetElement,
							...
							// On some Android devices activeElement needs to be
							// blurred otherwise the synthetic click will have
							// no effect (#24)
						...
						FastClick.prototype.determineEventType = function(
							...
							// Issue #159: Android Chrome Select Box does not
							// open with a synthetic click event
							if (this.deviceIsAndroid
						...
						FastClick.prototype.focus = function(targetElement) {
							...
							// Issue #160: on iOS 7, some input elements (e.g.
							// date datetime) throw a vague TypeError on
							// setSelectionRange. These elements don't have an
							// integer value for the selectionStart and
							// selectionEnd properties, but unfortunately that
							// can't be used for detection because accessing the
							// properties also throws a TypeError. Just check
						...
						FastClick.prototype.updateScrollParent = function(
							...
							// Attempt to discover whether the target element is
							// contained within a scrollable layer. Re-check if
							// the
						...
						FastClick.prototype.getTargetElementFromEventTarget = function(
							...
							// - see issue #56) the event target may be a text
							// node.
						...
						/**
						 * On touch start, record the position and scroll
						 * offset.
						 * 
						 * @param {Event}
						 *            event
						...
						FastClick.prototype.onTouchStart = function(event) {
							...
							// Ignore multiple touches, otherwise pinch-to-zoom
							// is prevented if both fingers are on the FastClick
							// element (issue #111).
							...
							if (this.deviceIsIOS) {
								...
								if (!this.deviceIsIOS4) {
									...
									// Weird things happen on iOS when an alert
									// or confirm dialog is opened from a click
									// event callback (issue #23):
									// when the user next taps anywhere else on
									// the page, new touchstart and touchend
									// events are dispatched
									// with the same identifier as the touch
									// event that previously triggered the click
									// that triggered the alert.
									// Sadly, there is an issue on iOS 4 that
									// causes some normal touch events to have
									// the same identifier as an
									// immediately preceeding touch event (issue
									// #52), so this fix is unavailable on that
									// platform.
									...
									// scrollable layer (using
									// -webkit-overflow-scrolling: touch) and:
									// 1) the user does a fling scroll on the
									// scrollable layer
									// 2) the user stops the fling scroll with
									// another tap
									// then the event.target of the last
									// 'touchend' event will be the element that
									// was under the user's finger
									// when the fling scroll was started,
									// causing FastClick to send a click event
									// to that layer - unless a check
									// is made to ensure that a parent layer was
									// not scrolled before sending a synthetic
									// click (issue #42).
							...
							// Prevent phantom clicks on fast double-tap (issue
							// #36)
						...
						/**
						 * Based on a touchmove event object, check whether the
						 * touch has moved past a boundary since it started.
						 * 
						 * @param {Event}
						 *            event
						 * @returns {boolean}
						...
						/**
						 * Attempt to find the labelled control for the given
						 * label element.
						 * 
						...
						FastClick.prototype.findControl = function(labelElement) {
							...
							// Fast path for newer browsers supporting the HTML5
							// control attribute
							if (labelElement.control !== undefined) {
								return labelElement.control;
							}

							// All browsers under test that support touch events
							// also support the HTML5 htmlFor attribute
							if (labelElement.htmlFor) {
								return document
										.getElementById(labelElement.htmlFor);
							}

							// If no for attribute exists, attempt to retrieve
							// the first labellable descendant element
						...
						/**
						 * On touch end, determine whether to send a click event
						 * at once.
						 * 
						 * @param {Event}
						 *            event
						...
						FastClick.prototype.onTouchEnd = function(event) {
							...
							// On some iOS devices, the targetElement supplied
							// with the event is invalid if the layer
							// is performing a transition or scroll, and has to
							// be re-detected manually. Note that
							// for this to function correctly, it must be called
							// *after* the event target is checked!
							// See issue #57; also filed as rdar://13048589 .
							if (this.deviceIsIOSWithBadTarget) {
								touch = event.changedTouches[0];

								// In certain cases arguments of
								// elementFromPoint can be negative, so prevent
								// setting targetElement to null
							...
							if (targetTagName === 'label') {
								...
								// Case 1: If the touch started a while ago
								// (best guess is 100ms based on tests for issue
								// #36) then focus will be triggered anyway.
								// Return early and unset the target element
								// reference so that the subsequent click will
								// be allowed through.
								// Case 2: Without this exception for input
								// elements tapped when the document is
								// contained in an iframe, then any inputted
								// text won't be visible even though the value
								// attribute is updated as the user types (issue
								// #37).
								...
								// Select elements need the event to go through
								// on iOS 4, otherwise the selector menu won't
								// open.
								if (!this.deviceIsIOS4
							...
							if (this.deviceIsIOS && !this.deviceIsIOS4) {
								...
								// Don't send a synthetic click event if the
								// target element is contained within a parent
								// layer that was scrolled
								// and this tap is being used to stop the
								// scrolling (usually initiated by a fling -
								// issue #42).
								scrollParent = targetElement.fastClickScrollParent;
								if (scrollParent
										&& scrollParent.fastClickLastScrollTop !== scrollParent.scrollTop) {
							...
							// Prevent the actual click from going though -
							// unless the target node is marked as requiring
							// real clicks or if it is in the whitelist in which
							// case only non-programmatic clicks are permitted.
						...
						FastClick.prototype.onMouse = function(event) {
							...
							// If a target element was never set (because a
							// touch event was never fired) allow the event
							...
							// Programmatically generated events targeting a
							// specific element should be permitted
							if (!event.cancelable) {
								return true;
							}

							// Derive and check the target element to see
							// whether the mouse event needs to be permitted;
							// unless explicitly enabled, prevent non-touch
							// click events from triggering actions,
							...
							if (!this.needsClick(this.targetElement)
								...
								if (event.stopImmediatePropagation) {
									...
									// support Event#stopImmediatePropagation
									// (e.g. Android 2)
							...
							// If the mouse event is permitted, return true for
						 ...
						 * naturally after a delay after a touch (which needs to
						 * be cancelled to avoid duplication), or an actual
						 * click which should be permitted.
						 * 
						 * @param {Event}
						 *            event
						 * @returns {boolean}
						...
						FastClick.prototype.onClick = function(event) {
							...
							// delivered with third-party code to fire a click
							// event before FastClick does (issue #44). In that
							// case, set the click-tracking flag back to false
							// and return early. This will cause onTouchEnd to
							// return early.
							if (this.trackingClick) {
								this.targetElement = null;
								this.trackingClick = false;
							...
							// Very odd behaviour on iOS (issue #18): if a
							// submit element is present inside a form and the
							// user hits enter in the iOS simulator or clicks
							// the Go button on the pop-up OS keyboard the a
							// kind of 'fake' click event will be triggered with
							...
							// Only unset targetElement if the click is not
							// permitted. This will ensure that the check for
							// !targetElement in onMouse fails and the browser's
							// click doesn't go through.
							if (!permitted) {
								this.targetElement = null;
							}

							// If clicks are permitted, return true for the
						...
						FastClick.notNeeded = function(layer) {
							...
							// Devices that don't support touch don't need
							// FastClick
							...
							if (chromeVersion) {
								...
								if (FastClick.prototype.deviceIsAndroid) {
									...
									if (metaViewport) {
										...
										// width=device-width or less don't need
										// FastClick
										if (chromeVersion > 31
	...
	require
			.register(
					...
					function(exports, require, module) {
						...
						/**
						 * Switchery 0.5.1 http://abpetkov.github.io/switchery/
						 * 
						 * Authored by Alexander Petkov
						 * https://github.com/abpetkov
						 * 
						 * Copyright 2013-2014, Alexander Petkov License: The
						 ...
						 * 
						 * @param {Boolean}
						 *            clicked - we need this in order to uncheck
						 *            the input when the switch is clicked
						 * @api private
						 */
